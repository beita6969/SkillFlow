"""
准备 v3 训练数据集 — 7 个 IID 数据集，每个 500 训练 + 128 评估

IID 数据集（训练+评估）:
  1. HotpotQA      (multi_hop_qa)     — 有 context passages
  2. AIME           (math_reasoning)   — 竞赛数学
  3. ALFWorld       (interactive_agent) — 家庭环境交互
  4. WebShop        (interactive_agent) — 网购交互
  5. TriviaQA       (factual_qa)       — 有 wiki/search context
  6. SWE-bench      (code_generation)  — 有 code_files workspace
  7. MedQA          (science_qa)       — 医学多选题

OOD 数据集（只评估，另行准备）:
  MuSiQue, MATH-Hard, Sokoban, WebArena, NQ, HumanEval, GPQA
"""
import json
import re
import random
...
# ══════════════════════════════════════════════════════════════
# 1. HotpotQA (多跳QA) — 有 context passages
# ══════════════════════════════════════════════════════════════
print("1. HotpotQA...")
# distractor split: 答案 95% 在 context 中（vs fullwiki 只 60%）
...
# ══════════════════════════════════════════════════════════════
# 3. ALFWorld (交互式Agent) — 需要 env_type/env_config
# ══════════════════════════════════════════════════════════════
print("3. ALFWorld...")
items = []
task_templates = [
...
# ══════════════════════════════════════════════════════════════
# 4. WebShop (购物Agent) — 需要 env_type/env_config
# ══════════════════════════════════════════════════════════════
print("4. WebShop...")
...
# ══════════════════════════════════════════════════════════════
# 5. TriviaQA (直接QA) — 用 rc split（有 wiki_context）
# ══════════════════════════════════════════════════════════════
print("5. TriviaQA...")
items = []
for pf in sorted(_glob.glob(str(BASE / "triviaqa/rc/validation-*.parquet"))):
    ...
    for _, row in df.iterrows():
        ...
        # 提取 context passages（wiki_context + search_context）
        passages = []
...
# ══════════════════════════════════════════════════════════════
# 6. SWE-bench (代码) — 从 patch 提取 code_files
# ══════════════════════════════════════════════════════════════
print("6. SWE-bench...")
...
def _ensure_repo_cloned(repo: str) -> Path:
    """确保 repo 已 clone（shallow clone 节省空间）"""
...
def extract_files_from_repo(repo: str, base_commit: str, file_paths: list) -> dict:
    """从 git repo 的指定 commit 提取完整文件内容"""
    import subprocess
    code_files = {}
    repo_dir = _REPO_CACHE_DIR / repo.replace("/", "__")
    ...
    for fp in file_paths[:3]:  # 最多 3 个文件
        try:
...
def extract_pre_patch_files(patch_text, repo="", base_commit=""):
    """从 git repo 提取完整文件，或从 patch 提取 context 作为 fallback"""
    # 从 patch 获取受影响的文件列表
...
for i, (_, row) in enumerate(df.iterrows()):
    ...
    # 从 repo clone 提取完整文件（优先），fallback 到 patch context
...
# ══════════════════════════════════════════════════════════════
# 7. MedQA (医学QA) — IID for science_qa
# ══════════════════════════════════════════════════════════════
print("7. MedQA...")
items = []
